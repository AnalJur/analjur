...
function ClienteCard({ cliente, onEdit, onClick }: {
  cliente: Cliente;
  ...
  return (
    <div
      ...
      className="bg-bg border border-border rounded-2xl p-5 hover:border-gold/40 hover:shadow-lg transition-all cursor-pointer group">
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        <button
          ...
          className="opacity-0 group-hover:opacity-100 transition-opacity text-xs text-muted hover:text-text-main border border-border rounded-lg px-2.5 py-1 hover:border-gold/40 flex-shrink-0">
          Editar
        </button>
      </div>
      ...
      <div className="space-y-1.5 text-xs text-muted">
        {cliente.email && (
          <p className="flex items-center gap-2 truncate">
            <span>✉️</span> <span className="truncate">{cliente.email}</span>
          </p>
        )}
        {cliente.telefone && (
          <p className="flex items-center gap-2">
            <span>📞</span> <span>{cliente.telefone}</span>
          </p>
        )}
        {cliente.ultima_movimentacao && (
          <p className="flex items-center gap-2">
            <span>🕒</span> <span>Última mov. {fmtData(cliente.ultima_movimentacao)}</span>
          </p>
        )}
      </div>
    </div>
        ...
        <Modal title="Editar Cliente" onClose={() => setModal(null)}>
          <div className="space-y-4">
            ...
            <div className="border-t border-border pt-3">
              <button
                onClick={() => { setModal(null); deletar(modal); }}
                disabled={deletando === modal.id}
                className="text-xs text-danger hover:underline disabled:opacity-50">
                {deletando === modal.id ? "Excluindo…" : "Excluir este cliente"}
              </button>
            </div>
          </div>
        </Modal>
