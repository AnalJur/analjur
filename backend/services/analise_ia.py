...
Cada tipo de análise tem um prompt especializado.
Outputs sempre em JSON estruturado para persistência e revisão humana.
"""
...
from ..models import Analise, Peca, Cronologia, TarefaRevisao
...
JSON schema esperado:
{
  "riscos": [
    {
      "categoria": "prazo | probatorio | legal | estrategico | financeiro",
      "descricao": "string",
...
{
  "teses_autor": [
    {"tese": "string", "fundamento_legal": "string", "status": "acolhida | pendente | rejeitada", "evidencia": "string"}
  ],
  "teses_reu": [
    {"tese": "string", "fundamento_legal": "string", "status": "acolhida | pendente | rejeitada", "evidencia": "string"}
  ],
...
JSON schema esperado:
{
  "acoes": [
    {
      "acao": "string",
      "tipo": "prazo_legal | diligencia | peticao | audiencia | recurso",
      "urgencia": "normal | alta | urgente | critica",
...
async def gerar_analise(
    ...
    """Gera análise de IA e cria registro com status 'pendente' para revisão humana."""
    ...
    # Se for cronologia, persiste também na tabela cronologia
    if tipo == "cronologia" and "eventos" in conteudo:
        for ev in conteudo["eventos"]:
            ...
            if ev.get("data"):
                try:
                    data = date_type.fromisoformat(ev["data"])
                except ValueError:
                    pass
            cron = Cronologia(
                processo_id=processo_id,
    ...
    # Cria tarefa de revisão humana automaticamente
...
async def chat_processo(
    ...
    """Chat livre sobre o processo com RAG. Retorna (resposta, fontes, tokens)."""
